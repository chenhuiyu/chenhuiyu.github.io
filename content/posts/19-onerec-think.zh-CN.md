---
title: "推荐也需要先想后答吗？｜生成式推荐 19：OneRec-Think"
date: "2026-07-29"
updated: "2026-07-29"
...
slug: "generative-recommendation-19-onerec-think-zh"
excerpt: "OneRec-Think 先将商品 code 对齐到语言空间，再用推荐 CoT 与 Rollout-Beam GRPO 激活推理，并以 Think-Ahead 把慢思考拆到离线。"
...
[MTGR](/blog/generative-recommendation-18-mtgr-zh) 让工程特征与大模型骨干融合，但决策仍是隐式的。[OneRec-Think: In-Text Reasoning for Generative Recommendation](https://arxiv.org/abs/2510.11639) 首次系统地把文字推理插入工业生成式推荐：
...
![从隐式生成到先想后答](/blog/generative-recommendation/19-onerec-think/paradigm-shift.svg)
...
\[
s_{v_{n+1}}
\sim P(\cdot\mid\mathcal P(S_u),\tau;\theta),
\]

其中 \(\tau=(r_1,\ldots,r_M)\) 是 reasoning tokens，\(s_{v_{n+1}}\) 是目标 itemic tokens。

![八件商品与实时意图](/blog/generative-recommendation/19-onerec-think/toy-reasoning.svg)
...
4. 把得到的 rationale 当监督，再用**完整原始历史**训练模型生成 rationale 与目标。
...
![OneRec-Think 三阶段与部署](/blog/generative-recommendation/19-onerec-think/mechanism.svg)
...
![Itemic Alignment 与 Reasoning 消融](/blog/generative-recommendation/19-onerec-think/evidence.svg)
...
[系列终篇 OneReason](/blog/generative-recommendation-20-onereason-zh) 将从这个失败出发，把推荐推理拆成 R0–R3，设计四粒度预训练、结构化 CoT 与“先专后合”RL，并给出一组罕见的反证：SFT 后多想反而更差，只有经过特定 RL 后 thinking 才稳定胜出。
